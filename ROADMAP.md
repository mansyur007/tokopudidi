...
>
> **Verifikasi produksi (2026-08-29)** — tiga acceptance yang sejak Juli ditandai "cek pasca-deploy" akhirnya diperiksa langsung terhadap https://toko.emha.space. **M12-D3 lulus**: Google Rich Results Test menjawab "3 valid items detected". **M12-D4 tetap terbuka**, tapi sekarang ada bukti terukurnya — gambar utama produk dirender 158 css px tapi menerima berkas 600×600 (29 KB), dan sebabnya cara Next menyusun srcset untuk `sizes` bersatuan `vw` (kandidat mulai 640w, tanpa kandidat kecil). **M15-D1 tetap terbuka**: seluruh bahan installability terbukti benar di produksi (HTTPS, manifest, keempat ikon berdimensi sungguhan, theme color cocok), yang belum bisa dibuktikan hanya prompt Chrome Android & angka Lighthouse — keduanya butuh alat yang tidak ada di lingkungan ini.
>
> **Progress (2026-08-29, lanjutan)** — **M9-C1 scope kategori** (satu-satunya acceptance M9 yang masih terbuka sejak 8 Juli) selesai: `PromoCode.categoryId` + penegakan per item + pemilih kategori di admin. Kuncinya kategori itu **pohon** dan produk hidup di daunnya, jadi scope voucher wajib mencakup seluruh turunan — kalau tidak, voucher untuk kategori induk tidak pernah kena apa pun. Ikut disederhanakan: pembagian diskon per order dari tiga cabang jadi satu aturan (proporsional terhadap subtotal yang **berhak**).
>
> **Progress (2026-08-29)** — **M14-A2 Email Transaksional selesai**, dan statusnya yang selama ini ⚪ BLOCKED ternyata **salah**: verifikasi item ini memang dirancang lewat **MailHog** (SMTP palsu di docker-compose), bukan penyedia berbayar — kredensial asli baru perlu saat cutover produksi. Label itu menahan item M selama empat minggu tanpa sebab. Di luar rencana tapi wajib: **jalur pengisian email** (opsional di register, `PATCH /auth/me`, kartu Email di /akun) — tanpa itu `User.email` tidak punya satu pun cara diisi (produksi: 9 dari 10 user tanpa email) dan kelima event akan diam selamanya. Sisa antrean: **M14-A1** (masih butuh `GOOGLE_CLIENT_ID` sungguhan), **OPS-5…OPS-8, OPS-10b…OPS-12**.
>
> **Progress (2026-08-06)** — **OPS-10 dipecah** jadi **10a Uptime check (🟢 DONE)** dan **10b Error tracking & log terpusat (🔵 TODO)**, karena bagian keduanya butuh akun eksternal sementara bagian pertama tidak — menyatukan keduanya di satu baris berarti item ini tidak akan pernah bisa ditandai selesai. 10a: workflow `uptime.yml` memanggil `scripts/uptime-check.sh` tiap ~15 menit dari GitHub (bukan dari dalam VPS seperti smoke-test OPS-3, jadi ia juga menangkap DNS mati, sertifikat kedaluwarsa, dan Caddy tak terjangkau dari luar). Logikanya ditaruh di script supaya bisa diuji terhadap host yang sengaja salah — dan memang diuji begitu, bukan diasumsikan.
>
> **Progress (2026-08-05, sore)** — **OPS-9** selesai: gate `tsc` kembali. Yang mengejutkan, **tidak ada type error yang perlu dibereskan** — judul itemnya mengasumsikan utang type di api yang menumpuk, padahal diukur ulang `tsc --noEmit` api & web sama-sama **0 error** dan `next lint` bersih; utangnya sudah lunas diam-diam lewat pekerjaan milestone, hanya bypass-nya yang tidak pernah dicabut. Jadi: `ignoreBuildErrors`/`ignoreDuringBuilds` dilepas dari `next.config.js`, api ikut di-`tsc` di Dockerfile dan produksi menjalankan `node dist/index.js` (bukan `npx tsx src/index.ts`), CI dapat step `Typecheck api` + `Build api`. Keempat gerbangnya dibuktikan bergigi lewat probe yang disuntik lalu dicabut. Sisa antrean: **OPS-5…OPS-8, OPS-10…OPS-12** dan M14-A1/A2 (⚪ BLOCKED). Catatan jujur: image Docker belum di-build di sini (daemon mati) — **pantau deploy pertama pasca-merge**.
>
...
- **Deliver notes tambahan** (2026-08-29): **scope kategori akhirnya dikerjakan** — kolom `PromoCode.categoryId` (nullable, `onDelete: SetNull`), penegakan per item di checkout & `/promo/validate`, dan pemilih kategori di halaman admin voucher.
  **Yang menentukan desainnya**: kategori berbentuk **pohon** (`Category.parentId`) dan produk hidup di **daun**-nya. Voucher kategori yang dicocokkan persis dengan `product.categoryId` karena itu praktis tidak pernah kena apa pun — "Voucher Elektronik" tidak berlaku untuk produk di "Elektronik › Handphone", satu-satunya tempat produknya ada. Jadi scope-nya diperluas ke seluruh turunan lewat `expandCategoryTree` di `packages/shared` (9 unit test), lengkap dengan penjaga **siklus**: tidak ada constraint database yang mencegah A→B→A, dan penelusuran naif akan menggantung selamanya — gejalanya di produksi berupa request yang tidak pernah selesai, bukan error yang bisa dibaca.
  **Penyederhanaan yang ikut terjadi**: pembagian diskon ke tiap order dulu punya tiga cabang (voucher toko penuh / platform proporsional / tidak ada). Sekarang satu aturan — proporsional terhadap `basePerShop`, yaitu subtotal yang **berhak** per toko. Voucher toko jatuh keluar sebagai kasus khusus dengan sendirinya (petanya cuma berisi satu toko), dan voucher kategori otomatis tidak memberi potongan ke toko yang tak punya item di kategori itu. Tanpa ini, membagi proporsional terhadap subtotal toko akan mendiskon barang yang tidak pernah masuk promo.
  **Di luar rencana**: (1) `POST /promo/validate` kini **butuh auth** — dasar diskon voucher kategori dihitung dari keranjang di server, dan angka `subtotal` kiriman klien sengaja tidak dipakai untuk kasus ini supaya diskonnya tidak bisa dikarang; (2) `categoryId` **tidak** ditambahkan ke `voucherCreateSchema` yang dipakai route seller melainkan ke varian admin tersendiri — field yang diterima lalu diabaikan diam-diam akan membuat seller mengira vouchernya ter-scope padahal tidak; (3) admin route menolak `categoryId` yang tidak menunjuk kategori mana pun, karena FK-nya `SetNull`: id ngawur tidak ditolak database melainkan diterima lalu diam-diam jadi null, dan voucher yang dikira ter-scope berubah jadi berlaku untuk semua; (4) seed diberi `SEMBAKO10` supaya cabang ini benar-benar terpakai di dev & e2e.
  **Dibuktikan bergigi**: melumpuhkan penyaringan di `/promo/validate` membuat TC-195 & TC-196 merah (TC-197 tetap hijau — lapisan berbeda); melumpuhkan guard di checkout membuat `SEMBAKO10` diterima untuk keranjang **tanpa satu pun item Sembako** dengan diskon Rp 9.500, dan setelah probe dicabut panggilan yang sama ditolak "Voucher ini hanya untuk produk kategori Sembako".
  **Bug yang ditangkap e2e-nya sendiri di CI**: versi pertama menghitung basis diskon dari `product.price` mentah dengan alasan "harga daftar cukup untuk menilai kelayakan". Untuk produk yang sedang diskon, basisnya jadi lebih besar daripada yang benar-benar ditagih — `/validate` menjanjikan potongan yang tidak akan direproduksi checkout (TC-195 di CI: 6.800 vs 4.700). Sekarang memakai `getUnitPrice` + harga flash + modifier varian, helper yang sama dengan keranjang & checkout; dan ekspektasi TC-195 dihitung dari harga yang benar-benar dilihat pembeli di keranjang, bukan dari harga kartu.
  **Batas jujur**: kategori seed semuanya datar (tanpa anak), jadi jalur pewarisan induk→anak terbukti lewat unit test, bukan lewat e2e. TC-TKPDD-195–197 perlu didaftarkan di TestForge.
- **Acceptance**:
  - [x] Voucher tanpa shopId muncul di Voucher Picker untuk semua user
  - [x] Bisa target scope kategori (opsional `categoryId`) — TC-195 (diskon hanya dari item kategori itu), TC-196 (ditolak kalau kategorinya tak ada di keranjang), TC-197 (checkout konsisten)
- **Effort**: S
...
- **Acceptance**:
  ...
  - [x] **Google Rich Results Test pass** — dijalankan 2026-08-29 terhadap `https://toko.emha.space/produk/kopi-robusta-lampung-250gr-toko-m`: **"3 valid items detected"**, crawl sukses. Product snippets ✓, Review snippets ✓, Merchant listings ✓ (dengan _non-critical issues_, yaitu field opsional merchant — tidak menggugurkan kelayakan). Verifikasi ini memang baru bisa dilakukan setelah ada URL publik
...
- **Acceptance**:
  ...
  - [ ] **Lighthouse "Properly size images" pass di halaman produk & ulasan** — **masih terbuka, dan sekarang ada bukti terukurnya**: diperiksa langsung di produksi 2026-08-29, gambar utama halaman produk dirender **158 css px** tapi menerima berkas **600×600 (29 KB)** — 3,8× lebih besar dari yang dibutuhkan pada DPR 1. Sebabnya bukan `sizes` yang lupa dipasang, melainkan cara Next menyusun srcset: gambar ber-`sizes` satuan **px tetap** (kartu, avatar toko) mendapat 16 kandidat mulai **16w**, sedangkan gambar utama yang memakai `(max-width:1080px) 90vw, 340px` hanya mendapat 8 kandidat **mulai 640w** — jadi slot sempit tidak punya kandidat kecil untuk dipilih berapa pun `sizes`-nya. Perbaikan yang mungkin (mengganti term `vw` jadi px bertingkat) **sengaja belum dikerjakan**: dampaknya ke ketajaman di layar ber-DPR tinggi perlu dikonfirmasi dengan Lighthouse sungguhan, dan alat itu belum bisa dijalankan dari sini
- **Effort**: S
...
- **Status**: 🟢 DONE · **Owner**: Claude
- **Deliver notes** (2026-08-29): **Status BLOCKED-nya keliru, dan spesifikasi item ini sendiri yang membantahnya.** Yang tertulis di bagian Implementation — "ENV kosong → log-only mode (dev tidak wajib MailHog)" dan verifikasi lewat MailHog — semuanya bisa dijalankan tanpa satu pun kredensial: MailHog itu SMTP palsu, bukan penyedia email. Kredensial asli baru dibutuhkan saat **cutover produksi**, bukan saat membangun; menandai item ini BLOCKED menahannya selama empat minggu tanpa sebab. Yang benar-benar butuh akun eksternal cuma pengiriman ke inbox sungguhan, dan itu memang belum dilakukan.
  **Temuan yang mengubah lingkup**: rencana menyebut event "Welcome saat register dengan email terisi" — padahal `registerSchema` **tidak pernah menanyakan email** dan tidak ada satu pun endpoint yang bisa mengisi `User.email` (tidak ada halaman/route profil). Di produksi: **9 dari 10 user tanpa email**. Artinya kelima event akan `return` di baris pertama selamanya dan fitur ini ter-deploy dalam keadaan mati. Karena itu di sini ditambahkan jalur pengisiannya: `email` opsional di register (schema + form) dan `PATCH /api/v1/auth/me` + kartu Email di `/akun`. Tanpa itu item ini "selesai" tapi tidak pernah mengirim apa pun.
  **Keputusan implementasi**: `sendMail` mengembalikan `void`, bukan Promise — supaya pemanggil **tidak bisa** meng-`await` dan menahan checkout di belakang SMTP; kegagalan berhenti jadi baris log (dibuktikan: SMTP mati → register tetap 201 dalam 361 ms, `ECONNREFUSED` muncul di pino belakangan). Transport diberi `connectionTimeout`/`greetingTimeout`/`socketTimeout`: fire-and-forget tanpa timeout menumpuk socket sampai proses kehabisan handle. `auth` hanya dipasang kalau `SMTP_USER` ada — MailHog tanpa auth akan menolak AUTH yang dikirim dengan kredensial `undefined`. Email dikirim **setelah transaksi commit**, bukan di dalamnya: email untuk keputusan yang kemudian di-rollback tidak bisa ditarik kembali. Seluruh interpolasi lewat `escapeHtml` — isi email dirakit dari nama toko/produk/catatan admin yang diketik orang lain, dan klien email merender HTML. COD ditandai PAID sejak dibuat, jadi email "sudah dibayar" untuk COD dikirim dari checkout, bukan dari jalur pembayaran. Verifikasi transfer oleh seller **tidak** mengirim email ke seller — dia sendiri yang barusan menekannya.
  **Yang ketahuan karena sengaja dilumpuhkan**: probe pertama (mematikan `notifyOrderCreated`) tidak membuat TC-191 merah — ternyata proses API lama masih memegang port 4000 dan `pkill` tidak menyentuhnya di Windows, jadi yang diuji sepanjang itu adalah biner tanpa probe. Setelah proses yang benar dimatikan, TC-191 merah dengan sebab yang tepat ("inbox berisi 0") sementara 189/190/192 tetap hijau. Percobaan pertama TC-194 juga sempat menumpang refund PENDING dari seed — begitu run pertama memakainya, run berikutnya "lolos" sebagai **skip tanpa menguji apa pun**; sekarang test itu membuat pesanannya sendiri sampai DELIVERED lalu mengajukan refundnya.
  **Terverifikasi lokal (2026-08-29)**: Docker + Postgres + MailHog, **6/6 e2e email (TC-189–194)** dan **81/82 e2e suite penuh**; `tsc` api+web+shared, `lint` bersih, **318 unit test** (30 baru). Satu-satunya merah adalah **TC-153 (admin log)** yang **tidak berhubungan**: filter tanggal server memakai hari **lokal** (`setHours` lokal) sementara test menghitung "hari ini" dengan `toISOString()` (UTC), jadi ia merah hanya di mesin yang tanggal lokalnya berbeda dari UTC — persis kondisi jam 00:00–07:00 WIB. Di runner CI (UTC) hijau. Diperbaiki terpisah.
  **Batas jujur**: yang belum dibuktikan adalah pengiriman ke penyedia sungguhan (Resend/Brevo) dan tampilannya di klien email nyata — produksi tetap **log-only** sampai `SMTP_HOST` diisi di `/opt/tokopudidi/.env.production`. Event komplain-diputus dites di level template + satu baris call site, bukan e2e (jalur eskalasinya panjang); saudara terdekatnya, refund-diputus, dites end-to-end. TC-TKPDD-189–194 perlu didaftarkan di TestForge.
...
- **Acceptance**:
  - [x] Dev + MailHog: 5 event di atas tertangkap dengan isi benar — TC-189 (welcome), TC-191 (pesanan dibuat), TC-193 (dibayar → toko, dikirim+resi → pembeli), TC-194 (refund diputus). Komplain-diputus memakai jalur & template yang sama, diuji unit
  - [x] ENV SMTP kosong → app jalan normal, email cuma ke log — register 201, log `email (log-only, SMTP_HOST kosong)`
  - [x] SMTP down → checkout/aksi tetap sukses, error email di pino — port mati: register 201 dalam 361 ms, `ECONNREFUSED` muncul di log belakangan
  - [x] User tanpa email → tidak ada error, tidak ada kiriman — TC-190
- **Effort**: M
...
- **Status**: 🟢 DONE · **Owner**: Claude
- **Deliver notes** (2026-08-02): **Rencana salah baca soal prioritas.** "Flash > sale > grosir" kalau dibaca harfiah berarti harga flash Rp 60.000 mengalahkan tier grosir Rp 45.000 — pembeli yang memborong justru ditagih lebih mahal, bertentangan dengan kontrak `min` yang sudah ditulis M13-B1 dan dengan arti kata "grosir" itu sendiri. Yang ditegakkan: **yang dibayar selalu kandidat termurah**, dan urutan prioritas hanya menentukan siapa yang dilaporkan saat angkanya seri. Karena itu `resolveUnitPrice` mengembalikan `{ price, source }`, dan `source` bukan hiasan — **kuota flash hanya dipotong kalau harga flash yang benar-benar menang**; kalau tier grosir lebih murah, slot flash-nya tidak ikut terbakar oleh transaksi yang tidak memakainya. **Perubahan struktur yang tidak ada di rencana**: harga checkout kini dihitung **di dalam** `$transaction`, bukan sebelumnya. Sejak ada kuota, harga sebuah item baru pasti setelah slotnya dipesan, dan slot bisa keburu diambil orang lain — menghitung subtotal di luar transaksi membuat `Order.total` bisa tidak sama dengan jumlah itemnya tepat pada kasus balapan. Validasi promo ikut masuk karena diskonnya berbasis subtotal itu. **Pemesanan kuota pakai SQL mentah, bukan pola `updateMany` di rencana**: syaratnya membandingkan dua kolom (`soldCount + qty <= quota`) dan Prisma tidak bisa menyusun itu; membaca `quota` lebih dulu lalu `soldCount: { lte: quota - qty }` menyisakan celah kalau admin menurunkan kuota di antara baca dan tulis. **Penghapusan slot/event dijaga lewat keberadaan `OrderItem`, bukan `soldCount`** — `soldCount` yang sudah kembali 0 tidak berarti tidak ada yang menunjuk ke sana (baris pesanan yang batal tetap ada), dan menghapusnya membuat pembatalan berikutnya kehilangan sasaran pelepasan kuota. **Pemeriksaan tumpang tindih menyertakan event yang dijeda dan ikut jalan saat periode event diubah** — tanpa yang kedua, aturannya ditegakkan di pintu masuk slot lalu bocor lewat pintu edit event. **Di luar rencana tapi wajib**: harga flash dipasang ke kartu lewat `applyFlashPrices` (satu pasca-proses atas `ProductCard[]`, bukan lookup di enam `include` — pelajaran `CARD_SHOP_SELECT` dari M14-B1, dengan taruhan lebih besar: badge beda antar halaman itu membingungkan, harga beda antar halaman itu tidak bisa dipercaya), dan keranjang + halaman detail ikut memakainya supaya angka sebelum menekan Bayar sama dengan yang ditagih. **Ketahuan dari e2e**: hitungan mundur `HH:MM:SS` polos merender event tujuh hari sebagai "165:30:47"; formatnya kini `formatSisaWaktu` di shared (dipakai bersama section beranda & BuyBox). TC-155 ikut diperbarui — opsi filter jejak audit 22 → 28, tripwire literal-nya bekerja persis seperti maksud penulisnya. **Test balapan sempat tidak bergigi, dan itu ketahuan hanya karena penjaganya sengaja dilumpuhkan.** Versi pertama TC-178 memakai `Promise.all` atas satu `APIRequestContext` — permintaannya ternyata terserialisasi, jadi checkout kedua sudah membaca kuota nol dan lewat jalur sekuensial; test-nya tetap hijau walau syarat `soldCount + qty <= quota` dibuang. Perbaikannya dua lapis: satu bagian deterministik (beli 2 saat kuota 1) yang tidak bergantung waktu sama sekali, dan konteks HTTP terpisah untuk bagian bersamaannya. Sekali lagi ketahuan bahwa "hijau" bukan bukti sampai pernah dilihat merah karena sebab yang benar. **Terverifikasi lokal (2026-08-02)**: `docker compose up -d` → `prisma migrate reset` → `prisma migrate deploy` → `db:seed` → **65/65 e2e lolos**, termasuk TC-174–178. Lolos juga: `tsc` api+web+shared+database, lint, 288 unit test (24 baru). TC-TKPDD-174–178 perlu didaftarkan di TestForge.
...
- **Acceptance**:
  - [x] Kuota habis → "Habis" di UI, checkout pakai harga normal tanpa error — bar kuota jadi abu-abu + "Kuota habis — harga kembali normal"; e2e TC-178 memastikan checkout-nya tetap 201
  - [x] Race kuota terakhir → tepat 1 dapat harga flash — e2e TC-178, **bukan** unit test: `payment.test.ts` justru dibuat khusus untuk bagian yang tidak bergantung DB, sedangkan yang diuji di sini adalah jaminan atomik Postgres. Dua bagian: satu deterministik (beli 2 saat kuota 1 → jatuh ke harga normal, `soldCount` tidak tersentuh) dan satu benar-benar bersamaan lewat dua `APIRequestContext` terpisah
  - [x] Order flash di-cancel/expired/refund → `soldCount` turun kembali (via titik `restoreStock`) — e2e TC-175
  - [x] Lewat `endAt` → section hilang tanpa intervensi; harga kembali normal — `runningWhere` menyaring saat baca, dan section beranda menghilangkan dirinya sendiri saat hitungan mundur habis tanpa perlu reload
  - [x] Prioritas: flash > sale (M9-B3) > grosir (M13-B1) — komentar kontrak di `price.ts` diperbarui, **dengan koreksi**: prioritas menentukan pemenang saat seri, bukan izin menaikkan harga (lihat deliver notes)
- **Effort**: L
...
### M15-B1. Pre-Order
- **Status**: 🟢 DONE · **Owner**: Claude
- **Deliver notes** (2026-08-02): Konsistensi `isPreorder`/`preorderDays` ditegakkan di ROUTE, bukan cuma zod — update parsial bisa mengirim salah satu tanpa yang lain, jadi server menggabungkan payload dengan data existing sebelum memutuskan: `isPreorder` aktif tanpa lama hari 1-90 ditolak 400, dan begitu hasil akhirnya BUKAN pre-order, `preorderDays` di-`null`-kan **tanpa syarat**. Badge `"📦 Pre-Order · N hari"` jadi komponen bersama (`PreorderBadge.tsx`) dipakai identik di `ProductCard`, `BuyBox`, keranjang, dan checkout — bukan empat salinan JSX, supaya tidak ada peluang teksnya berbeda antar halaman (pelajaran `CARD_SHOP_SELECT`/`applyFlashPrices` dari M14-B1/M15-C1). Field `isPreorder`/`preorderDays` mengalir lewat `include`/`select` Prisma yang sudah ada (yang perlu disentuh cuma `CardRow`/`ProductCard` di `product.service.ts` dan select eksplisit di `cart.service.ts`). Aturan estimasi tinggal di `packages/shared/src/utils/preorder.ts` (`maxPreorderDays`/`preorderEstimate`), bukan inline di JSX — lead time TERLAMA (bukan rata-rata), dihitung dari `paidAt`, atas snapshot `OrderItem.preorderDays` (bukan `Product.preorderDays` yang bisa sudah diubah seller).
- **Koreksi hasil audit (2026-08-02, Opus)** — tiga hal lolos di kiriman pertama dan sudah diperbaiki di PR yang sama:
  1. **Guard konsistensinya ternyata cuma satu arah.** Versi pertama memakai `else if (rest.isPreorder === false)`, sehingga `PATCH { preorderDays: 30 }` sendirian pada produk biasa tersimpan apa adanya — lalu `PATCH { isPreorder: true }` berikutnya lolos memakai angka yang tidak pernah diketik seller di sesi itu. Deliver notes lama menulis server "menggabungkan payload dengan data existing", padahal itu baru benar untuk satu arah. Sekarang `else` tanpa syarat, sekaligus menyembuhkan baris lama yang telanjur menyimpan sisa nilai.
  2. **Kriteria "campur ready + pre-order" sempat ditandai selesai tanpa satu pun test.** Rumusnya ditulis inline di JSX sehingga tidak bisa diuji tanpa merender halaman. Dipindah ke shared + 8 unit test + e2e TC-184 yang memesan 2 pre-order (3 & 10 hari) + 1 produk biasa dan memastikan tiap baris membawa snapshot-nya sendiri (yang biasa tetap `null`, tidak kebagian lead time tetangganya).
  3. **TC-183 menjanjikan lebih dari yang diuji** — judulnya menyebut "BuyBox & keranjang" tapi halaman keranjang tidak pernah dibuka. Sekarang benar-benar dibuka lewat `injectBuyerSession`. Plus: duplikasi produk (`POST /:id/duplicate`) tidak ikut menyalin setelan pre-order — salinan lahir mengaku ready stock; diperbaiki dan dikunci di TC-179.
  4. **TC-183 yang diperbaiki itu langsung menemukan bug pre-existing**: `/keranjang` (dan `/checkout`) membuang pembeli yang SUDAH login ke `/masuk` saat dibuka lewat URL langsung, karena `router.push('/masuk')` jalan sebelum `persist` selesai memulihkan sesi. Kelas yang sama dengan yang dibayar di `/pesanan/[id]`; `useAuthHydrated` sudah ada, dua halaman ini saja yang belum memakainya. **Hanya kelihatan di build produksi** — `next dev` lolos terus, `next start` gagal deterministik. Sebelas halaman buyer lain masih punya bug yang sama; di luar lingkup, dicatat terpisah.
  Penjaga barunya sudah dibuktikan bergigi: perbaikan (1) sengaja dilumpuhkan dan TC-180 memang merah dengan sebab yang benar (`Received: 30`), bukan sekadar hijau.
- **Terverifikasi lokal (2026-08-02)**: `docker compose up -d` → `prisma migrate reset` → `db:seed` → **71/71 e2e lolos** (TC-TKPDD-179–184). Lolos juga: `tsc` api+web+shared+database, lint, **296 unit test** (8 baru). Alur lengkap (toggle di form seller → badge di listing/detail/keranjang/checkout) diperiksa langsung di browser.
...
- **Acceptance**:
  - [x] Badge konsisten di card, detail, cart, checkout — komponen bersama `PreorderBadge`; e2e TC-181 (listing+detail) & TC-183 (BuyBox+keranjang), checkout diperiksa via browser
  - [x] Checkout campur ready + pre-order → estimasi pakai lead time terlama — `maxPreorderDays` di shared, 8 unit test + e2e TC-184
  - [x] Toggle off → `preorderDays` ter-clear di DB — ditegakkan di route tanpa syarat, bukan cuma zod (e2e TC-180, dibuktikan merah saat guard dilumpuhkan)
...
- **Acceptance**:
  - [ ] Chrome Android tawarkan "Add to Home Screen" — **masih belum terbukti**. Bahan-bahannya diperiksa langsung di produksi 2026-08-29 dan semuanya benar: HTTPS ✓, `manifest.webmanifest` 200 dengan `content-type: application/manifest+json` ✓, `display: standalone` ✓, `id: "/"` ✓, keempat ikon benar-benar terunduh dengan **dimensi piksel sungguhan** sesuai deklarasinya (192/512, any & maskable) ✓, dan `theme_color` manifest sama dengan `<meta name="theme-color">` (#1FA463) ✓. Yang tidak bisa dibuktikan dari sini: event `beforeinstallprompt` tidak terdengar dalam 3 detik di browser tertanam — dan itu **bukan bukti gagal**, karena pane tertanam memang lazim tidak mengirimkannya. Butuh Chrome Android sungguhan
  - [ ] Lighthouse installable check pass — **masih belum terbukti**, alasan sama: Lighthouse belum bisa dijalankan dari lingkungan ini
...
| `OPS-9` | **Bereskan type API → kembalikan `tsc` gate** (lepas `tsx` runtime & `ignoreBuildErrors`) | 🟢 DONE | 2026-08-05 · ternyata **nol** type error tersisa: yang dikerjakan mencabut bypass, bukan memperbaiki kode. API prod jalan dari `dist/` (`node dist/index.js`), CI dapat step `Typecheck api` + `Build api`. Batas jujur: ESLint _warning_ tetap tidak menggagalkan `next build` |
| `OPS-10a` | **Uptime check** (heartbeat produksi dari luar VPS) | 🟢 DONE | 2026-08-06 · [`uptime.yml`](.github/workflows/uptime.yml) tiap ~15 menit memanggil [`scripts/uptime-check.sh`](scripts/uptime-check.sh): `/api/health` (mencakup Postgres), homepage, dan sisa umur sertifikat. Batas: cron GitHub bukan penjadwal presisi & workflow terjadwal dinonaktifkan otomatis setelah repo 60 hari menganggur |
| `OPS-10b` | **Error tracking & log terpusat** (Sentry / agregasi log) | 🔵 TODO | butuh akun & DSN eksternal — tidak bisa dikerjakan tanpa itu. Sisa dari OPS-10 setelah 10a |
