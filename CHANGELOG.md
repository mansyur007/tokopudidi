...
- **Pre-order** (`M15-B1`) — produk ditandai pre-order dengan lead time (1-90 hari), murni informasi: **tidak ada SLA/auto-cancel otomatis**.
  ...
  - `maxPreorderDays`/`preorderEstimate` di `packages/shared` — 8 unit test; e2e `preorder.spec.ts` (TC-TKPDD-179–184).

### Notes
- **Konsistensi ditegakkan di route, bukan cuma zod.** Update produk itu parsial — payload bisa mengirim `isPreorder` tanpa `preorderDays`, atau sebaliknya. Server menggabungkan payload dengan data existing sebelum memutuskan: `isPreorder` aktif tanpa lama hari 1-90 ditolak 400, dan begitu hasil akhirnya bukan pre-order, `preorderDays` di-`null`-kan **tanpa syarat** — termasuk ketika payload sama sekali tidak menyentuh toggle-nya.
...
- **Campuran ready + pre-order pakai lead time TERLAMA**, bukan rata-rata: pesanan dikirim sebagai satu paket, jadi rata-rata akan menjanjikan tanggal yang sudah pasti dilewati sejak awal. Aturannya tinggal di shared, bukan inline di JSX — lihat catatan audit di bawah.

### Fixed (audit pra-merge, 2026-08-02)
Tiga hal lolos di kiriman pertama milestone ini dan diperbaiki di PR yang sama:
- **Guard konsistensi ternyata cuma satu arah.** Versi pertama memakai `else if (rest.isPreorder === false)`, jadi `PATCH { preorderDays: 30 }` sendirian pada produk biasa tersimpan apa adanya — lalu `PATCH { isPreorder: true }` berikutnya lolos memakai angka yang tidak pernah diketik seller di sesi itu. Yang lebih buruk dari bug-nya: catatan rilisnya sudah telanjur mengklaim server menggabungkan payload dengan data existing, padahal itu baru benar untuk satu arah. Sekarang `else` tanpa syarat, sekaligus menyembuhkan baris lama yang telanjur menyimpan sisa nilai.
- **Kriteria penerimaan "campur ready + pre-order" sempat ditandai selesai tanpa satu pun test yang membuktikannya.** Rumusnya ditulis inline di JSX halaman pesanan, sehingga satu-satunya cara mengujinya adalah merender halaman — dan konsekuensinya tidak ada yang diuji sama sekali. Dipindah ke `packages/shared/src/utils/preorder.ts`, ditutup 8 unit test, plus e2e TC-184 yang memesan dua pre-order (3 & 10 hari) bersama satu produk biasa dan memastikan tiap baris membawa snapshot-nya sendiri — yang biasa tetap `null`, tidak kebagian lead time tetangganya di pesanan yang sama.
- **TC-183 menjanjikan lebih dari yang diuji**: judulnya menyebut "BuyBox & keranjang", tapi halaman keranjang tidak pernah dibuka — padahal badge di keranjang adalah kriteria penerimaan tersendiri. Sekarang benar-benar dibuka lewat `injectBuyerSession`. Ditemukan juga bahwa duplikasi produk (`POST /:id/duplicate`) tidak menyalin setelan pre-order, jadi salinannya lahir mengaku ready stock — mudah lolos karena salinan memang lahir nonaktif, sehingga tidak ada yang melihat badge-nya hilang sampai produknya diaktifkan. Diperbaiki dan dikunci di TC-179.
- **Penjaganya dibuktikan bergigi, bukan sekadar hijau.** Perbaikan pertama sengaja dilumpuhkan lagi dan TC-180 memang merah dengan sebab yang tepat (`Received: 30`) — disiplin yang sama seperti TC-178 di M15-C1.
- **Guard hidrasi auth di `/keranjang` & `/checkout`** (bug pre-existing, ketemu gara-gara TC-183). Keduanya memanggil `router.push('/masuk')` di `useEffect` berdasarkan `user`, tanpa menunggu `persist` selesai memulihkan sesi — jadi pembeli yang SUDAH login dan membuka halaman itu lewat URL langsung (bookmark, tautan, refresh keras) dibuang ke halaman masuk. Kelas yang sama dengan yang dibayar untuk `/pesanan/[id]`; solusinya (`useAuthHydrated`) sudah ada di repo sejak itu, dua halaman ini saja yang belum memakainya. Di `/checkout`, cek `!hydrated` sengaja ditaruh **sebelum** layar "Belum ada item yang dipilih" — sebelum sesi terbaca keranjangnya memang masih kosong, dan pesan itu menuduh pembeli tidak memilih apa-apa padahal halamannya yang belum siap.
  **Hanya kelihatan di build produksi.** `next dev` lolos terus; `next build` + `next start` gagal deterministik (2/2 termasuk retry). Itu sebabnya verifikasi manual saya sebelumnya lewat: gejalanya sempat terlihat di browser dan keliru dianggap "halaman belum sinkron". Sebelas halaman buyer lain masih punya bug yang sama — di luar lingkup PR ini, dicatat sebagai pekerjaan terpisah.
- **Terverifikasi lokal (2026-08-02).** `docker compose up -d` → `prisma migrate reset` → `db:seed` → **71/71 e2e lolos**, termasuk TC-179–184. Lolos juga: `tsc` api + web + shared + database, lint, dan **296 unit test**. Alur lengkap (toggle di form seller → badge di listing/detail/keranjang/checkout) diperiksa langsung di browser. TC-TKPDD-179–184 perlu didaftarkan di TestForge.
